...
robot_batteries = {} # Dictionary to store latest battery charge: {serial_number: battery_charge}
mqtt_client = None
...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    # Subscribe to visualization and state topics for all robots
    for serial in robot_serial_numbers:
        ...
        if base_topic:
            ...
            print(f"Subscribed to {vis_topic} and {state_topic}")
        ...
        if serial_number in robot_serial_numbers:
            try:
                ...
                if topic_type == "visualization":
                    ...
                    if "agvPosition" in data and isinstance(data["agvPosition"], dict):
                         pos = data["agvPosition"]
                    ...
                    if "batteryState" in data and "batteryCharge" in data["batteryState"]:
                         robot_batteries[serial_number] = data["batteryState"]["batteryCharge"]
...
def update_plot(frame):
    ...
    for serial, plot_obj in scatter_plots.items():
        if serial in robot_positions:
            ...
            bat = robot_batteries.get(serial, 100.0)
            text_labels[serial].set_text(f"{serial[-2:]} ({bat}%)")
            text_labels[serial].set_position((x, y + 0.2))
            updates.append(plot_obj)
            updates.append(text_labels[serial])
        else:
             plot_obj.set_data([], [])
             text_labels[serial].set_text(serial[-2:])
