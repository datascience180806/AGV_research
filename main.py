...
class VehicleSimulator:
    ...
    def state_iterate(self) -> None:
        ...
        # --- Battery consumption ---
        if self.state.battery_state.battery_charge > 0:
            if self.state.driving:
                # Consume 0.01% per tick when driving (approx 0.2% per second)
                self.state.battery_state.battery_charge = max(0.0, round(self.state.battery_state.battery_charge - 0.01, 2))
            else:
                # Consume 0.001% per tick when idle (approx 0.02% per second)
                self.state.battery_state.battery_charge = max(0.0, round(self.state.battery_state.battery_charge - 0.001, 2))
        
        # If battery is empty, force driving to False, print message, and halt simulation iteration
        if self.state.battery_state.battery_charge <= 0:
            self.state.driving = False
            print(f"SIM ({self.config.vehicle.serial_number}): Battery empty! Simulation stopped.")
            return

        # Default driving to False at start of tick logic (will be set to True if moving)
        self.state.driving = False
