...
class GroqModelAdapter(BaseModelAdapter):
    """Adapter cho các LLMs chạy qua Groq API (Inference siêu tốc và miễn phí)"""
    ...
    def generate_orders(
        ...
        if not self.api_key:
            return ModelResponse(
                ...
                raw_response="",
                latency_ms=0.0,
                error_message="Groq API Key is missing. Please set GROQ_API_KEY in .env."
            )
        ...
        # 2. Gửi request tới Groq API (OpenAI compatible endpoint)
        ...
        payload = {
            ...
            "temperature": 0.1,
            "max_tokens": 2048,
            "response_format": {"type": "json_object"}  # Groq hỗ trợ JSON mode cực kỳ ổn định
        }
        ...
        try:
            ...
            if response.status_code != 200:
                return ModelResponse(
                    ...
                    error_message=f"Groq API returned status {response.status_code}: {response.text}"
                )
