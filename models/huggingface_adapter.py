...
class HuggingFaceModelAdapter(BaseModelAdapter):
    """Adapter cho các Open-Source LLMs chạy trên Hugging Face Serverless Inference API"""
    
    def __init__(self, api_key: Optional[str] = None, model_name: str = "Qwen/Qwen2.5-Coder-32B-Instruct"):
        # Nạp Token từ môi trường nếu không truyền vào
        if not api_key:
    ...
    def generate_orders(
        ...
        if not self.api_key:
            return ModelResponse(
                ...
                raw_response="",
                latency_ms=0.0,
                error_message="Hugging Face API Token is missing. Please set HF_TOKEN in .env."
            )
        ...
        # 2. Gọi API Hugging Face qua Endpoint tương thích OpenAI Chat Completions
        url = f"https://api-inference.huggingface.co/models/{self.model_name}/v1/chat/completions"
        headers = {
        ...
        try:
            ...
            if response.status_code != 200:
                return ModelResponse(
                    ...
                    error_message=f"HF Inference API returned status {response.status_code}: {response.text}"
                )
                
            res_json = response.json()
            raw_text = res_json["choices"][0]["message"]["content"]
            
            # Làm sạch định dạng markdown blocks nếu có
