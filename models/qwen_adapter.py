...
class QwenModelAdapter(BaseModelAdapter):
    """Adapter cho các dòng mô hình Qwen chạy qua Alibaba Cloud Model Studio (International API)"""
    ...
    def generate_orders(
        ...
        if not self.api_key:
            return ModelResponse(
                ...
                raw_response="",
                latency_ms=0.0,
                error_message="Qwen Cloud API Key is missing. Please set QWEN_API_KEY or ALIBABA_CLOUD_MODEL_API_KEY in .env."
            )
        ...
        # 2. Gửi request tới Alibaba Cloud Model Studio (International OpenAI-compatible endpoint)
        ...
        try:
            ...
            if response.status_code != 200:
                return ModelResponse(
                    ...
                    error_message=f"Alibaba Cloud API returned status {response.status_code}: {response.text}"
                )
